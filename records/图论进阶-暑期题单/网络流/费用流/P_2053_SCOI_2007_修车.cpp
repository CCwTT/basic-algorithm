...
void solve(){
    ll m,n;
    cin>>m>>n;
    ll src=0,snk=n+n*m+1;
    cost_flow flow(2+n+m*n);
    for(ll i=1;i<=n;i++){
        for(ll j=1;j<=m;j++){
            ll x;
            cin>>x;
            for(ll k=1;k<=n;k++){
                flow.add_edge(i,j*n+k,1,x*k);
            }
        }
    }
    for(ll i=1;i<=n;++i) flow.add_edge(src,i,1,0);
    for(ll i=1;i<=n*m;++i) flow.add_edge(i+n,snk,1,0);
    cout<<fixed<<setprecision(2)<<(1.0*flow.mcmf(src,snk).second/n)<<endl;
}
