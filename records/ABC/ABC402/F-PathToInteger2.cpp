...
//scale breaking down
//time complexity:O(n^2)*O(nlog(n))=O(n^3log(n)) @@@
#include <bits/stdc++.h>
using namespace std;
...
void solve(){
	...
	ll ans=0;
	vector<vector<set<ll>>> mem1(n+1,vector<set<ll>>(n+1));
	vector<vector<set<ll>>> mem2(n+1,vector<set<ll>>(n+1));
	auto helper=[&](ll x)->void{
		...
		function<set<ll>(ll,ll)> dfs1=[&](ll ex,ll ey)->set<ll>{
			...
			if(!mem1[ex][ey].empty()) return mem1[ex][ey];
			set<ll> st;
			if(ex-1>=1){
		...
		function<set<ll>(ll,ll)> dfs2=[&](ll sx,ll sy)->set<ll>{
			...
			if(!mem2[sx][sy].empty()) return mem2[sx][sy];
			set<ll> st;
			if(sx+1<=n){
