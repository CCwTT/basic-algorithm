    ...
    for(ll i=1;i<=n;i++){
        ...
        if(op==0){
            ...
            if(bel[l]==bel[r]){
                ...
                for(ll j=l;j<=r;j++){
                    if(a[j]+add[bel[j]]<x){
                        ans=max(ans,a[j]+add[bel[j]]);
                    }
                }
                cout<<(ans==LLONG_MIN/2?-1:ans)<<endl;
            }else{
                ll ans=LLONG_MIN/2;
                ll L=bel[(l+len-2)/len*len+1];
                ll R=bel[r/len*len];
                ...
                for(ll j=L;j<=R;j++){
                    auto it=lower_bound(vec[j].begin(),vec[j].end(),x-add[j]);
                    if(it==vec[j].begin()) continue;
                    ans=max(ans,*prev(it)+add[j]);
                }
                cout<<(ans==LLONG_MIN/2?-1:ans)<<endl;
            }
        }
