...
//@@@
//sgt(flip,binary),prefixization of range operation
#include<bits/stdc++.h>
using namespace std;
...
void solve(){
    ...
    while(m--){
        ...
        if(op==1){
            ...
            cin>>x;
            if(sgt.query_sum(1,1,n,x,x)==-1){
                cout<<0<<'\n';
                continue;
            }
            ll left=x,right=n,res=0;
            ll pre=(x==1)?0:sgt.query_min(1,1,n,x-1,x-1);
            while(left<=right){
                ll mid=(left+right)>>1;
                if(sgt.query_min(1,1,n,x,mid)>=pre){
                    res=mid;
                    left=mid+1;
                }else{
                    right=mid-1;
            ...
            if(res<n){
                ...
                ll ans=sgt.query_binary(1,1,n,x,n,pre);
                cout<<(ans!=x?ans:0)<<'\n';
            }
        }
    }
