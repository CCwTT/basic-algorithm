...
void solve(){
	...
	vector<ll> c(2*n+1),v(2*n+1);
	vector<ll> cnt(m+1);
	for(ll i=1;i<=2*n;i++){
		cin>>c[i];
		cnt[c[i]]++;
	}
	for(ll i=1;i<=2*n;i++) cin>>v[i];
	if(accumulate(l.begin()+1,l.end(),0ll)>n){
		cout<<-1<<endl;
		return;
	}
	ll base=0;
	ll src=0,snk=2*n+m+1;
	cf.clear();
	cf.n=2*n+m+2;
	for(ll i=1;i+1<=2*n;i++){
		cf.add_edge(i,i+1,(i+1)/2,-inf);
		cf.add_edge(i,i+1,i,0);
		base+=(i+1)/2*-inf;
	}
	cf.add_edge(2*n,snk,n,0);
	for(ll i=1;i<=m;i++){
		cf.add_edge(src,2*n+i,l[i],-inf);
		cf.add_edge(src,2*n+i,cnt[i]-l[i],0);
		base+=l[i]*-inf;
	}
	for(ll i=1;i<=2*n;i++){
		cf.add_edge(2*n+c[i],i,1,-v[i]);
	}
	auto [mf,mc]=cf.mcmf(src,snk);
	if(mc>base) cout<<-1<<endl;
	else cout<<-(mc-base)<<endl;
}
